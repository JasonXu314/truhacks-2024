...
export class AppGateway implements OnGatewayConnection<WebSocket>, OnGatewayDisconnect<WebSocket> {
	...
	@SubscribeMessage('SET_TEXT')
	public async setText(@MessageBody() text: string, @ConnectedSocket() client: WebSocket): Promise<ClientErrorDTO | void> {
		const data = this._resolve(client);
		...
		if (data === null) {
			...
			const msg = JSON.stringify({ type: 'SET_TEXT', text });
